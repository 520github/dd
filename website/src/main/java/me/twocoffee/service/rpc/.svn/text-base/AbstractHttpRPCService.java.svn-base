...
import me.twocoffee.filter.LocaleFilter.RequestHeaderHolder;
...
public abstract class AbstractHttpRPCService {
	...
	if (httpClientThreadLocal.get() == null) {
	    ...
	    defaultHttpClient.getParams().setParameter(
		    ...
		    RequestHeaderHolder.get("User-Agent"));
	...
	if (host == null) {
		...
		host = SystemConstant.domainName;
	    }
	}
	return host;
    }
	...
	try {
		...
		responseEntity = objectMapper.readValue(responseEntityStr,
			resultEntity);
	    }
	} catch (Exception e) {
	    throw e;
    ...
    protected void setAuthorization(Map<String, String> headerMap,
	    String authToken) {
	if (headerMap == null)
	    return;
    ...
    protected String getWebTokenAuthorization(String webToken) {
    	return TokenUtil.getWebTokenAuthorization(webToken);
    }
